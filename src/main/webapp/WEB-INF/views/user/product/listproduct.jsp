...
<body>
	...
	<div class="well well-small">
		...
		<c:if test="${ productByPaginate.size() > 0 }">
			<div class="row-fluid">
				<ul class="thumbnails">
					<c:forEach var="item" items="${ productByPaginate }"
						varStatus="loop">
						<li class="span4">
							<div class="thumbnail">
...
<!-- 									</div> -->
						...
						<c:if
							test="${ (loop.index + 1) % 3 == 0 || (loop.index + 1) == productByPaginate.size() }">
				</ul>
			</div>
			<c:if test="${ (loop.index + 1) < productByPaginate.size() }">
				<div class="row-fluid">
					<ul class="thumbnails">
			</c:if>
		</c:if>
	...
	<c:forEach var="item" begin="1" end="${ paginateInfo.totalPage }" varStatus="loop">
		<c:if test="${ (loop.index)  == paginateInfo.currentPage }">
			<a href="<c:url value="/search/${ searchThing }/${ loop.index }" />" class="active">${ loop.index }</a>
		</c:if>
		<c:if test="${ (loop.index)  != paginateInfo.currentPage }">
			<a href="<c:url value="/search/${ searchThing }/${ loop.index }" />">${ loop.index }</a>
		</c:if>
<!-- 		<a href="#">&laquo;</a> -->
<!-- 		<a href="#">&raquo;</a> -->
	</c:forEach>
